      ...
      <bim-toolbar class="self-start" vertical>
        <bim-toolbar-section>
          <bim-button @click=${onMeasurementsClick} ?active=${areMeasurementsEnabled} label="测量" tooltip-title="测量" icon=${appIcons.RULER}>
            <bim-context-menu>
              <bim-button ?active=${lengthMeasurer.enabled} label="长度" @click=${onLengthMeasurement}></bim-button>
              <bim-button ?active=${areaMeasurer.enabled} label="面积" @click=${onAreaMeasurement}></bim-button>
            </bim-context-menu>
          </bim-button>
          <bim-button ?active=${clipper.enabled} @click=${onModelSection} label="剖面" tooltip-title="模型剖面" icon=${appIcons.CLIPPING}></bim-button> 
        </bim-toolbar-section>
      </bim-toolbar>
    `;
  };
